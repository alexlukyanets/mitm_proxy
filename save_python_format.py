...
"""
    ...
    def request(self, flow: http.HTTPFlow):
        # if 'elcorteingles.es' not in flow.request.host:
        #     return
        if 'detectportal' in flow.request.host:
            return
        str_body_items = None
    ...
    #     if flow.response.text and not isinstance(flow.response.content, str):
    #         import gzip
    #         f = gzip.open('body2.gz', 'wb')
    #
    #         f.write(flow.response.content)
    #         f.close()
    #
    #         f = gzip.open('body1.gz', 'wb')
    #         f.write(flow.response.data)
    #         f.close()
    #
    #         f = gzip.open('body3.gz', 'wb')
